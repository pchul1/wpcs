...
<%-- 	</sec:authorize> --%>
	...
	function eduRegister(eduSeq)
	{
		...
		$.ajax({
			type: "POST",
			url: "<c:url value='/edu/mergeEduMember.do'/>",
			data: {eduSeq:eduSeq,
			...
			success : function(result){
				if (result.msg == 'alreadlyRegister') {
					alert('이미 신청하신 교육입니다.');
				} else if (result.msg == 'eduRegisterOK') { 
					alert('신청이 완료 되었습니다.');

					dataLoad();
					
				} else if (result.msg == 'overCapacity') {
					alert('교육정원 초과로 신청하실 수 없습니다.');
				}
			...
			success : function(result){
				...
				if( tot <= 0 ){
	            	...
	            	for(var i=0; i < tot; i++){
						...
						table += "<div align=\"right\">연락처 : <input type=\"text\" id=\"mobileNo\" style=\"width:150px;\">&nbsp;<img src=\"/images/common/btn_regist.gif\" alt=\"등록\" onclick=\"eduRegister('"+obj.eduSeq+"');\" style=\"cursor:pointer;vertical-align:middle;padding-bottom:3px;\"></div>";
						table += "<br/>";
