...
var $waterMap = function () {
	...
	var ARC_SERVER_URL = "http://210.99.81.159:8090/gis"
	var WKT = 3857;
	...
	var init = function () {
		...
		// 아래 PHY 를 V1 로 변경
		symbolColors['PHY'] = [66, 185, 244, 0.9];
		symbolColors['V2'] = [66, 185, 244, 0.9];
		symbolColors['V3'] = [66, 185, 244, 0.9];
		symbolColors['V4'] = [66, 185, 244, 0.9];
		symbolColors['V5'] = [66, 185, 244, 0.9];
		symbolColors['V6'] = [66, 185, 244, 0.9];	
		
		// PHY 범례값은 예외
		legendLevels['PHY'] = 0.0;
		legendLevels['V2'] = 10.0; // BOD
		legendLevels['V3'] = 50.0; // COD 
		legendLevels['V4'] = 40.0; // TP
		legendLevels['V5'] = 30.0; // TN
		legendLevels['V6'] = 4.0;  // SS
		...
		var initialExtent = new esri.geometry.Extent({
			xmax: 14563586.959531447,
			xmin: 13829791.48799379,
			ymax: 4588243.723795163,
			ymin: 4099046.7427700595,
		...
		map.on('zoom-end', function(event){
			...
			if($waterMap.zoomEnded){
				if(typeof($waterMap.zoomEnded) == 'function'){
					$waterMap.zoomEnded.apply(this, [currentDepth, event.level]);
				}
			}
		})
	...
	var getFeatures = function () {
		...
		query2.outFields = ['GIS_SD_PT.OBJECTID', 'WPCS.V_FACT_LOC_INFO_1D.DO_NM','WPCS.V_FACT_LOC_INFO_1D.TOTAL','WPCS.V_FACT_LOC_INFO_1D.NORMAL','WPCS.V_FACT_LOC_INFO_1D.A1','WPCS.V_FACT_LOC_INFO_1D.A2','WPCS.V_FACT_LOC_INFO_1D.A3','WPCS.V_FACT_LOC_INFO_1D.A4','WPCS.V_FACT_LOC_INFO_1D.M','WPCS.V_FACT_LOC_INFO_1D.PHY','WPCS.V_FACT_LOC_INFO_1D.V2','WPCS.V_FACT_LOC_INFO_1D.V3','WPCS.V_FACT_LOC_INFO_1D.V4','WPCS.V_FACT_LOC_INFO_1D.V5','WPCS.V_FACT_LOC_INFO_1D.V6'];
		query2.returnGeometry = true;
		query2.outSpatialReference = spatialReference;
		query2.where = '1=1';

		var query3 = new esri.tasks.Query();
		query3.outFields = ['GIS_SGG_PT.OBJECTID', 'GIS_SGG_PT.DO_NM', 'GIS_SGG_PT.CTY_NM','WPCS.V_FACT_LOC_INFO_2D.TOTAL','WPCS.V_FACT_LOC_INFO_2D.NORMAL','WPCS.V_FACT_LOC_INFO_2D.A1','WPCS.V_FACT_LOC_INFO_2D.A2','WPCS.V_FACT_LOC_INFO_2D.A3','WPCS.V_FACT_LOC_INFO_2D.A4','WPCS.V_FACT_LOC_INFO_2D.M','WPCS.V_FACT_LOC_INFO_2D.PHY','WPCS.V_FACT_LOC_INFO_2D.V2','WPCS.V_FACT_LOC_INFO_2D.V3','WPCS.V_FACT_LOC_INFO_2D.V4','WPCS.V_FACT_LOC_INFO_2D.V5','WPCS.V_FACT_LOC_INFO_2D.V6'];
		query3.returnGeometry = true;
		query3.outSpatialReference = spatialReference;
		query3.where = '1=1';

		var query4 = new esri.tasks.Query();
		query4.outFields = ['FACT_CODE', 'FACT_NAME', 'BRANCH_NO','MIN_TIME','MIN_OR','RIVER_DIV','SYS_KIND','ADM_CD','PHY','V2','V3','V4','V5','V6'];
	...
	var getRenderer = function (features) {
		...
		var outlineSymbol;
		// 아래 PHY 를 V1 로 변경
		if(currentFieldName == 'PHY' || currentFieldName.indexOf('V') == 0){
			outlineSymbol = new esri.symbol.SimpleLineSymbol().setWidth(3).setColor(new esri.Color([172, 173, 168,0.7]));	
		}else{
			outlineSymbol = new esri.symbol.SimpleLineSymbol().setWidth(0);	
		}
		...
		for(var i=0; i<features.length; i++){
			var value = features[i].attributes[currentFieldName];
			if(value == null){
				features[i].attributes[currentFieldName] = -1;
			}
		}
		
		// 아래 PHY 를 V1 로 변경
		d1Renderer.addBreak(-1, -1, new esri.symbol.SimpleMarkerSymbol(esri.symbol.SimpleMarkerSymbol.STYLE_CIRCLE,symbolSize, outlineSymbol,new esri.Color([123,123,123])));
		
		if(currentFieldName == 'PHY'){
			d1Renderer.addBreak(0, 5.9, new esri.symbol.SimpleMarkerSymbol(esri.symbol.SimpleMarkerSymbol.STYLE_CIRCLE,symbolSize, outlineSymbol,new esri.Color(symbolColors['A3'])));
			d1Renderer.addBreak(6, 9.5, new esri.symbol.SimpleMarkerSymbol(esri.symbol.SimpleMarkerSymbol.STYLE_CIRCLE,symbolSize,outlineSymbol,  new esri.Color(symbolColors[currentFieldName])));
			d1Renderer.addBreak(9.6, 9999, new esri.symbol.SimpleMarkerSymbol(esri.symbol.SimpleMarkerSymbol.STYLE_CIRCLE,symbolSize,outlineSymbol,  new esri.Color(symbolColors['A3'])));
		}else {
			var legendVal = legendLevels[currentFieldName];
			d1Renderer.addBreak(0, legendVal, new esri.symbol.SimpleMarkerSymbol(esri.symbol.SimpleMarkerSymbol.STYLE_CIRCLE,symbolSize, outlineSymbol,new esri.Color(symbolColors[currentFieldName])));
			d1Renderer.addBreak(legendVal+0.1, 9999.0, new esri.symbol.SimpleMarkerSymbol(esri.symbol.SimpleMarkerSymbol.STYLE_CIRCLE,symbolSize,outlineSymbol,  new esri.Color(symbolColors['A3'])));
		}
	...
	var refreshFeatureLayers = function (fieldName) {
		...
		if(currentDepth != 'd3'){
			...
			for (var i = 0; i < featureDatas[currentDepth].length; i++) {
				var feature = featureDatas[currentDepth][i];
				var value = feature.attributes[currentFieldName];
			...
			for (var i = 0; i < featureDatas[currentDepth].length; i++) {
				var feature = featureDatas[currentDepth][i];
				var value = feature.attributes[titleFieldNm];
				if(value == null){
					value = '';
				}
				var symbol = new esri.symbol.TextSymbol({
