...
public class ExcelDetailViewFlux extends AbstractExcelView{
	@Override
	protected void buildExcelDocument(Map model, HSSFWorkbook wb,
		...
		HSSFSheet sheet = wb.createSheet("수위, 유량 조회 상세조회 결과");
		...
		if (chart.size() > 0) {
			...
			isVO = obj instanceof FlowDataVO;
		}
		...
		for (int i = 0; i < chart.size(); i++) {
			...
			if (isVO) {	// VO
				...
				FlowDataVO vo = (FlowDataVO) chart.get(i);
