 ...
 *
 */
public class ExcelStatsPreventView extends AbstractExcelView{
	...
	protected void buildExcelDocument(Map model, HSSFWorkbook wb,
		...
		cell = getCell(sheet, 3, 6);
		setText(getCell(sheet, 3, 6), menu[7]);
		cell.setCellStyle(style);
		
		cell = getCell(sheet, 3, 7);
		setText(getCell(sheet, 3, 7), menu[8]);
		cell.setCellStyle(style);
		
		cell = getCell(sheet, 3, 8);
		setText(getCell(sheet, 3, 8), menu[9]);
		cell.setCellStyle(style);		
		...
		for (int i = 0; i < chart.size(); i++) {
			...
			if (isVO) {	// VO일경우
				...
				StatsVO vo = (StatsVO) chart.get(i);
				...
				cell.setCellValue(vo.getVal3());
				cell.setCellStyle(style);
				
				cell = getCell(sheet, 4 + i, 8);
				cell.setCellValue(vo.getVal4());
				cell.setCellStyle(style);				
			} else {	// Map일경우(미사용)
