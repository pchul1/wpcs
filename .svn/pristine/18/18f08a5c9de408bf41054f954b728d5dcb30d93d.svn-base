...
<% 
 /**
  * Class Name : EgovFileListNew.jsp
  * Description : 파일 목록화면
...
<script type="text/javascript">
	...
	function fn_egov_deleteFile(atchFileId, fileSn) {
		...
		form.action = "<c:url value='/cmmn/deleteFileInfs.do'/>";
		form.submit();
	}
		...
		<tr>
			...
			<c:choose>
				<c:when test="${updateFlag=='Y'}">
					...
					<img src="<c:url value='/images/util/bu5_close.gif'/>" style="vertical-align:bottom;" onClick="fn_egov_deleteFile('<c:out value="${fileVO.atchFileId}"/>','<c:out value="${fileVO.fileSn}"/>');">
				</c:when>
				<c:otherwise>
					<a href="javascript:fn_egov_downFile('<c:out value="${fileVO.atchFileId}"/>','<c:out value="${fileVO.fileSn}"/>')">
						<c:out value="${fileVO.orignlFileNm}"/>&nbsp;[<c:out value="${fileVO.fileMg}"/>&nbsp;byte]
