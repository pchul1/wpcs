...
function addTreeNode(parentNode, recursedNodes) {
	for (var i = 0; i < treeNodes.length; i++) {
		...
		if (nodeValues[1] == parentNode) {
			...
			document.write("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;");
			document.write("<input type='checkbox' id='menuAuthor"+i+"' name='checkField' class='check2' ");
			if(nodeValues[4] == 1){ document.write(" checked "); }
			document.write("onclick='javascript:fCheckDir(this);' value=" + nodeValues[0] + " uppervalue=" + nodeValues[1] + ">");
			if (hasChildNode) {
 ...
 * 모두선택 처리 함수
 ******************************************************** */
function fCheckDir(n){
	
	var _parent = $(n);
	var _checked = $(n).attr("checked");
	var _parent_uppervalue = $(n).attr("uppervalue");
...
//		
//		$("#" + _parent_id + " > input[type='checkbox']").attr("checked",(_checked == undefined) ? false : true);
//		
//		if($(_parent).prop("tagName").toUpperCase() == "TR".toUpperCase()) {
//			i = 3;
//		}else{
//			i = 1;
//		}
