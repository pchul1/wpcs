	...
	$.fn.scrollShow = function( settings ){
		...
		return this.each(function(){
			var 
				...
				limit	  = $(settings.elements, $view).length,
				active	  = 0;
			...
			function sequential( event ){
				if (limit < $(settings.elements, $view).length){
					$elements = $(settings.elements, $view);
					limit = $(settings.elements, $view).length;
				}
			...
			function random( event ){
				...
				if( pos < 0 )
					pos = active == 0 && settings.circular ? limit-settings.size : 0;
				else if( pos > limit-settings.size )
					pos = active == limit && settings.circular ? 0 : limit-settings.size;
				

				if(limit > settings.size){
