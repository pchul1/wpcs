...
var imgpath			= ROOT_PATH + "images/util/";
var treeYeobu		= false;
...
function ViewcreateTree(arrName, vYeobu) {
//	console.log(arrName);
	var startNode, openNode;
	treeNodes = arrName;
	treeYeobu = vYeobu;
	if (treeNodes.length > 0) {
		...
		var recursedNodes = new Array();
		ViewaddTreeNode(startNode, recursedNodes);
	}
}
...
//	console.log(parentNode, recursedNodes,treeNodes);
	for (var i = 0; i < treeNodes.length; i++) {
		...
		if (nodeValues[1] == parentNode) {
			...
			var isNodeOpen = true;
			if(nodeValues[3].toLowerCase() == "false"){
				isNodeOpen = false;
			}
			...
			if (hasChildNode) {
				...
				nodscript = true;
				document.write("<input type='checkbox' name='menu_id' value='"+nodeValues[0]+"'");
				if (isNodeOpen) document.write("checked='checked'");
				document.write("> &nbsp;<a href='#' onclick=\"treecheckboxopen('"+nodeValues[0]+"'); return false;\">");
				document.write("<img id='icon" + nodeValues[0] + "' src='"+imgpath+"menu_page.gif' border='0' align='absbottom' alt='Page'>");
				document.write("</a>");
			}
...
function ViewaddTreeNode(parentNode, recursedNodes) {
//	console.log(parentNode, recursedNodes,treeNodes);
	for (var i = 0; i < treeNodes.length; i++) {
		var nodeValues = treeNodes[i].split("|");
		if (nodeValues[1] == parentNode) {
			...
			if (hasChildNode) {
				document.write("<div id='div" + nodeValues[0] + "'>");
				ViewaddTreeNode(nodeValues[0], recursedNodes);
				document.write("</div>");
			}
			recursedNodes.pop();
		}
...
function treecheckboxopen(n){
	if($("input[value='" + n + "']").attr("checked") == "checked"){
