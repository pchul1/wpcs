...
function fnSave() {
	...
	if(confirm(message))
	{
		frm = document.frm1;
		if($("#param6").val() == "2"){
			frm.alertStep.value = "6"; //상황종료
		}else{
			frm.alertStep.value = "4"; //경보발령
		}
		$("#analTime").val($("#Hour").val() + ":" + $("#Min").val());
		$("#procEndTime").val($("#procEndHour").val() + ":" + $("#procEndMin").val());
		
		frm.analGov.value = $("#param1 > option:selected").text().replace(">","-");
		frm.analPerson.value = $("#param2 > option:selected").text();
...
<div> 
	...
	<input type="hidden" name="analGov" id="param1_hidden"/>
		...
		<div class="write">
			<table> 
				...
				<tr>
					...
					<td>
						<c:if test='${isReview == false}'>
							<input type="text" name="analPhone" id="param3"/>
						</c:if>
						<c:if test="${isReview == true}">
				...
				<tr>
					...
					<td>
						<c:if test='${isReview == false}'>
							<input type="text" id="analDate" datepicker='Y' name="analDate" size="15" readonly="readonly" style="width:80px"/>
							<select id="Hour" name="Hour"  style="width:20%;"></select>시
							<select id="Min" name="Min"  style="width:20%;"></select>분
							<input type="hidden" class="inputText" size="4" name="analTime" id="analTime"/>
						</c:if>
						<c:if test="${isReview == true}">
		...
		<c:if test='${isReview == false}'>
			...
			<div style="float:left; width:50%;text-align:center;">
				<ul class="sbtn">
					<li style="width:90%"><a href="#" onclick="fnSave(); return false;">경보발령 >></a></li>
				</ul>
			</div>
		</c:if>
	</div>
