...
{
	Statement stmt=null;
	sql = "SELECT ADM_CD,A.BRANCH_NO,A.FACT_CODE,FACT_NAME,MIN_OR,MIN_TIME,PHY,RIVER_DIV,SYS_KIND,V2,V3,V4,V5,V6,X,Y FROM V_FACT_LOC_INFO_NEW A, (SELECT OBJECTID, FACT_CODE, BRANCH_NO,X,Y FROM GIS_AUTO WHERE USE_FLAG = 'Y' UNION SELECT OBJECTID, FACT_CODE, BRANCH_NO,X,Y FROM GIS_TMS WHERE 1=1 UNION SELECT OBJECTID, FACT_CODE, BRANCH_NO,X,Y FROM GIS_IPUSN WHERE 1=1 AND USE_FLAG = 'Y') B WHERE A.FACT_CODE = B.FACT_CODE AND A.BRANCH_NO = B.BRANCH_NO";
