...
sql+="TO_CHAR(TO_DATE(A.INPUT_TIME, 'YYYYMMDDHH24MI'), 'YYYY-MM-DD HH24:MI') UPDATE_TIME,";
sql+="A.STATUS_FLAG, A.BATTERY, B.FACT_CODE, B.BRANCH_NO, B.BRANCH_FNAME, B.BRANCH_NAME FROM T_IPUSN_GPS A, T_FACT_BRANCH_INFO B, (SELECT FACT_CODE, BRANCH_NO, MAX(INPUT_TIME)INPUT_TIME FROM T_IPUSN_GPS GROUP BY FACT_CODE,BRANCH_NO)C WHERE A.FACT_CODE = B.FACT_CODE AND A.BRANCH_NO = B.BRANCH_NO AND STATUS_FLAG='N' AND B.BRANCH_USE_FLAG='Y' AND A.FACT_CODE=C.FACT_CODE AND A.BRANCH_NO = C.BRANCH_NO AND A.INPUT_TIME = C.INPUT_TIME AND  SUBSTR( TO_NUMBER(A.INPUT_TIME),1,8) > '20110105' AND SUBSTR( TO_NUMBER(A.INPUT_TIME),1,8) <= '" + ObjectUtil.getTimeString("yyyyMMdd") + "'";
//System.out.println("getipusn >>>>>> " + sql);
