...
<%
	...
	try{
		...
		if(fileName==null){
			...
			File desti = new File("C:/water_temp");
			if(!desti.exists()){
				desti.mkdirs(); 
			}
			...
			if(svgInfo!=null && !"".equals(svgInfo)){
				BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("C:/water_temp/" + svgFileName), "UTF8"));
			    writer.write(svgInfo);
			    writer.flush();
			    writer.close();
// 			    String svg_URI_input = Paths.get("C:/temp/" + svgFileName).toUri().toURL().toString();
 			    String svg_URI_input = "file:C:/water_temp/" + svgFileName;
		        TranscoderInput input_svg_image = new TranscoderInput(svg_URI_input);     
			    
			    OutputStream png_ostream = new FileOutputStream("C:/water_temp/" + svgPngFileName);
			    ...
			    png_ostream.flush();
			    png_ostream.close();  
			    svgImg = ImageIO.read(new File("C:/water_temp/" + svgPngFileName));
			}
		    ...
		    File outputfile = new File("C:/water_temp/" + resultPngFileName);
...
// 		    }
			...
			File file = new File("C:/water_temp/" + fileName);
