...
//					console.log("window.location.href : ", window.location.href);
					...
					for ( var i = 0; i < data.layers.length; i++)
					{
						...
						html += '<li>'+
									'<label><input type="checkbox" class="list_item" id="'+l.layerId+'" onclick="$kecoMap.model.updateLayerVisibility();"/><img src="'+url+l.layerId+'/images/'+l.url+'" alt=""/> '+l.layerName+'</label>'+
								'</li>';
						if(l.layerId == '7')
						{
							html += '</ul>';
...
//						$('#41').attr('checked', true);
		...
		pub.addMarker = function(type, x, y, obj, flag) // type = 0 사고지점 , 1 = 방제창고, 2 = 진입지점 , 3 = 이탈 USN, 4 = 실시간, 5 = 주제도 
		{
			...
			//page.view.markerLayer.setStyle(style);
...
//				console.log(obj, data[obj]);
		...
		pub.mouseOverOnFeature = function(event, data){
			 ...
			 if(feature){
				 ...
				 if(featureInfo.featureType == 'MARKER'){
					 //pub.markerOverlay
					 if(featureInfo.featureTypeNumber == 0 || featureInfo.featureTypeNumber >= 10 || featureInfo.featureTypeNumber <= 14){
						 //featureInfo.markerIndex
						 //infoTemplate = new esri.InfoTemplate(EVENT_TEMP);
						 html = EVENT_TEMP.content;
					 }else if(featureInfo.featureTypeNumber == 1){
					 	 //infoTemplate = new esri.InfoTemplate("${지점명}","${*}");
						 html = "${지점명}","${*}";
					 }else if(featureInfo.featureTypeNumber == 2){
					 	 //infoTemplate = new esri.InfoTemplate("${branch_no}","${*}");
						 html = "${branch_no}","${*}";
					 }else if(featureInfo.featureTypeNumber == 3){
					 	 //infoTemplate = new esri.InfoTemplate(OUT_TEMP);
						 html = OUT_TEMP.content;
					 }else if(featureInfo.featureTypeNumber == 4){
					 	 //infoTemplate = new esri.InfoTemplate(CR_TEMP);
					...
					/*_CoreMap.tempBranchTooltipElement.innerHTML = feature.getProperties().properties.TITLE;
					_CoreMap.tempBranchToolTip.setPosition( feature.getGeometry().getCoordinates() );
					_CoreMap.tempBranchTooltipElement.classList.remove('hidden');*/
				 ...
				 //_CoreMap.tempBranchToolTip.setPosition(undefined);
			 }
		};
