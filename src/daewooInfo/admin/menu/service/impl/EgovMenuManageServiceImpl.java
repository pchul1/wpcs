...
//		vSiteMapName  = menuSiteMapvo.getTmp_rootPath()+"/"+menuSiteMapvo.getBndeFileNm();
	...
	private boolean siteMapCreat(String vSiteMapName, String vHtmlValue)throws Exception{
		...
		try {    
	        ...
	        //e.printStackTrace();
			log.error("EgovMenuManageServiceImpl.java Error-01:파일 생성 에러");
		} catch (IOException e) {                                            
	        //e.printStackTrace();
			log.error("EgovMenuManageServiceImpl.java Error-02:파일 열기 에러");
		} finally {
			try {
				out.close();   
				success = true; 
			} catch (IOException e) {
				log.error("EgovMenuManageServiceImpl.java Error-03:파일 닫기 에러");
			}
		}
		return  success ;   
	}
