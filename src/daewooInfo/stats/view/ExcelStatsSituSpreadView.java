...
public class ExcelStatsSituSpreadView extends AbstractExcelView{
	@Override
	protected void buildExcelDocument(Map model, HSSFWorkbook wb,
		...
		filename = "stats_situ_spread" + "-"+ctime+".xls";
		...
		cell = getCell(sheet, addIdx, cellIdx);
		sheet.addMergedRegion(new CellRangeAddress(addIdx,addIdx,cellIdx,cellIdx+1));
		setText(cell, "상황전파");
		cell = getCell(sheet, addIdx, cellIdx);
		cell.setCellStyle(style);
		...
		for (int i = 0; i < data.size(); i++) {
			...
			if (isVO) {	// VO
				...
				//SMS
				cell = getCell(sheet, addIdx + i, cellIdx++);
				cell.setCellValue(vo2.getSmsCnt());
				cell.setCellStyle(style);				
			
				//ACS
				cell = getCell(sheet, addIdx + i, cellIdx++);
				cell.setCellValue(vo2.getAcsCnt());
				cell.setCellStyle(style);				
